#!/bin/bash
# ───────────────────────────────────────────────────────────
#  Zerberus – sauberes Weitergabe-Paket bauen
#
#  Anlass (30.07.2026): Ein mit dem Finder gepacktes ZIP ging an einen
#  Prüfer — darin steckte die ECHTE Datenbank: Passwort-Hash, 49 Regeln mit
#  Geschäftsadressen, 66 Protokolleinträge mit Absendern und Betreffzeilen.
#  Gleichzeitig FEHLTEN .env.example, .dockerignore und .gitignore, weil der
#  Finder versteckte Dateien ausließ. Genau falsch herum also.
#
#  Dieses Skript macht es andersherum und prüft das Ergebnis nach:
#    RAUS: data/ (Datenbank!), Wissen/, .env, .venv/, .git/, staticfiles/,
#          __pycache__, *.pyc, .DS_Store, ._*  — und kein __MACOSX
#    REIN: .env.example, .dockerignore, .gitignore, Dockerfile, compose …
#
#  Der Wächter am Ende ist der eigentliche Zweck: Findet er eine .env, eine
#  Datenbank oder einen Wissen-Ordner im fertigen Paket, wird es GELÖSCHT
#  statt ausgeliefert.
# ───────────────────────────────────────────────────────────
...
clear
echo "════════════════════════════════════════════════"
echo "   📦  Zerberus – Weitergabe-Paket"
echo "════════════════════════════════════════════════"
echo ""
...
ZIEL="$ABLAGE/zerberus-$VERSION.zip"
echo "   Quelle:  $QUELLE"
...
echo "$INHALT" | grep -E '^(\.env\.example|\.dockerignore|\.gitignore|Dockerfile|docker-compose\.yml|README\.md|CHANGELOG\.md|manage\.py|requirements\.txt)$' \
  | sed 's/^/      /'
echo ""
echo "════════════════════════════════════════════════"
echo "   ✅  Liegt auf dem Schreibtisch:"
echo "       zerberus-$VERSION.zip"
echo ""
